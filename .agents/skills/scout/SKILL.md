...
`ask` runs search (with adaptive reranking) and generates a cited answer from
the top chunks with the Qwen3 1.7B chat model (`models/Qwen3-1.7B-Q4_K_M.gguf`,
target `chat` in the download scripts):

```powershell
uv run python -m src.document_indexer ask "How do I declare a variable in C#?" `
  --db-path data\lancedb
```

The chat model loads lazily and is cached like the reranker. In `shell`, prefix
a query with `/ask` to get an answer. Sources that exceed `--chat-context`
(8192) are truncated or dropped; `--answer-max-tokens` (512) caps the reply.
...
- `POST /ask`: same payload as `/query`; returns `query`, `answer`, and the
  cited `sources` generated by the chat model.
